    ...
    const fetchSymbols = async (str: any) => {
        try {
            ...
            if (response.data['Information']) {
                ...
                setSymbols(response.data.bestMatches.map((x: any) => ({ 
                    name: x['2. name'],
                    currency: x['8. currency'],
                    label: `${x['2. name']} (${x['1. symbol']})`, 
                    value: x['1. symbol'] 
                })))
            }
            setLoading(false)
        ...
        try {
            ...
            if (response.data['Information'] || response.data['Note']) {
                ...
                setOps({
                    ...
                    volume: Math.round(resp.reduce((total: number, x: any) => total + Number(series[x]['5. volume']), 0) / resp.length)
                })
                setCategories(resp)
                setData([
                    {
            ...
            <div className="flex justify-between">
                <div className="flex text-left">
                    <div>
                        ...
                        <h5 className="leading-none text-3xl flex items-end font-bold text-gray-900 dark:text-white pb-2"><p className="text-base font-normal mr-3 text-gray-500 dark:text-gray-400">{ symbol?.currency }</p>{ops?.open}</h5>
                    </div>
                    <div className="ml-6">
                        <p className="text-base font-normal text-gray-500 dark:text-gray-400">Closing</p>
                        <h5 className="leading-none text-3xl flex items-end font-bold text-gray-900 dark:text-white pb-2"><p className="text-base font-normal mr-3 text-gray-500 dark:text-gray-400">{ symbol?.currency }</p>{ops?.closing}</h5>
                    </div>
                    <div className="ml-6">
                        <p className="text-base font-normal text-gray-500 dark:text-gray-400">Volume</p>
                        <h5 className="leading-none text-3xl flex items-end font-bold text-gray-900 dark:text-white pb-2"><p className="text-base font-normal mr-3 text-gray-500 dark:text-gray-400">{ symbol?.currency }</p>{ops?.volume}</h5>
                    </div>
                </div>
                <div className="px-2.5 py-0.5 text-base font-semibold text-green-500 dark:text-green-500 text-left">
                    <p className="text-base font-normal text-gray-500 dark:text-gray-400">{info?.lastRefreshed}</p>
                    <h5 className="leading-none text-3xl font-bold text-gray-900 dark:text-white pb-2">{symbol?.name} ({symbol?.value})</h5>
                </div>
            </div>
            <Chart
                options={options}
